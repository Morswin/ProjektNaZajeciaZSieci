...
bool porownanieSekwencji(std::vector<double>& spodz, std::vector<double>& fakt)
{
    ...
    for (int i = 0; result && i < static_cast<int>(fakt.size()); i++)
...
{
    //Sygnatura testu:
    std::cerr << "ModelARX ({-0.4}, {0.6}) -> test zerowego pobudzenia: ";
    try
    {
        // Przygotowanie danych:
        ModelARX instancjaTestowa({ -0.4 }, { 0.6 }, 1, false);
...
{
    //Sygnatura testu:
    std::cerr << "ModelARX ({-0.4}, {0.6}) -> test skoku jednostkowego nr 1: ";
...
{
    //Sygnatura testu:
    std::cerr << "ModelARX ({ -0.4, 0.2 }, { 0.6, 0.3 }, 2) -> test skoku jednostkowego nr 2: ";
    try
    {
        // Przygotowanie danych:
        ModelARX instancjaTestowa({ -0.4, 0.2 }, { 0.6, 0.3 }, 2, false);
        constexpr size_t LICZ_ITER = 30;
        std::vector<double> sygWe(LICZ_ITER);      // pobudzenie modelu,
        std::vector<double> spodzSygWy(LICZ_ITER); // spodziewana sekwencja wy
        std::vector<double> faktSygWy(LICZ_ITER);  // faktyczna sekwencja wy
...
void test_ModelARX_zmienneOpoznienie()
{
    //Sygnatura testu:
    std::cerr << "ModelARX ({-0.4}, {0.6}) -> test skoku jednostkowego nr 1: ";
    try
